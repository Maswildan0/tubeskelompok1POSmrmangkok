...
class KeranjangController extends Controller
{
    ...
    public function daftarmenu()
    {
        ...
        $makanan = Menu::whereHas('kategori', function ($q) {
            $q->where('nama_kategori', 'makanan');
        })->get();
        
        $minuman = Menu::whereHas('kategori', function ($q) {
            $q->where('nama_kategori', 'minuman');
        })->get();
        
        $sambal = Menu::whereHas('kategori', function ($q) {
            $q->where('nama_kategori', 'sambal');
        })->get();
        
        $topping = Menu::whereHas('kategori', function ($q) {
            $q->where('nama_kategori', 'topping');
        })->get();
        
        $alaCarte = Menu::whereHas('kategori', function ($q) {
            $q->where('nama_kategori', 'ala carte');
        })->get();
        
        $paketHemat = Menu::whereHas('kategori', function ($q) {
            $q->where('nama_kategori', 'paket hemat');
        })->get();
    ...
    public function tambahKeranjang(Request $request)
    {
        ...
        try {
            ...
            try{
                $product = Menu::find($request->product_id); //ambi data barang simpan di tabel product
                if (!$product) {
    ...
    public function lihatkeranjang(){
        ...
        $menu = DB::table('penjualan')
                        ...
                        ->join('pembeli', 'penjualan.pembeli_id', '=', 'pembeli.id')
                        ->select('penjualan.id','penjualan.no_faktur','pembeli.nama_pembeli', 'penjualanmenu.menu_id', 'menu.nama','penjualanmenu.harga_jual', 
                                 'menu.gambar','pembayaran.order_id',
                                  DB::raw('SUM(penjualanmenu.jml) as total_menu'),
                                  DB::raw('SUM(penjualanmenu.harga_jual * penjualanmenu.jml) as total_belanja'))
                        ->where('penjualan.pembeli_id', '=',$id_pembeli) 
                        ->where(function($query) {
                            $query->where('pembayaran.gross_amount', 0)
                                  ->orWhere(function($q) {
                        ...
                        ->groupBy('penjualan.id','penjualan.no_faktur','pembeli.nama_pembeli','penjualanmenu.menu_id', 'menu.nama','penjualanmenu.harga_jual',
                                  'menu.gambar','pembayaran.order_id',
                                 )
                        ->get();

        // hitung jumlah total tagihan
        $ttl = 0; $jml_mn = 0; $kode_faktur = '';
        foreach($menu as $p){
            $ttl += $p->total_belanja;
            $jml_mn += 1;
    ...
    public function hapus($menu_id)
    {
        ...
        $sql = "DELETE FROM penjualanmenu 
                WHERE menu_id = ? 
                AND penjualan_id IN (
                    SELECT penjualan.id 
                    FROM penjualan 
                    JOIN pembayaran 
                        ON penjualan.id = pembayaran.penjualan_id 
                    ...
                    ";
        $deleted = DB::delete($sql, [$menu_id,$id_pembeli]);
    ...
    public function lihatriwayat(){
        ...
        $detail_menu = DB::table('penjualanmenu')
            ->join('menu', 'penjualanmenu.menu_id', '=', 'menu.id')
        ...
        return view('riwayat',
                        [ 
                            ...
                            'total_tagihan' => $ttl,
                            'total_belanja' => $t->total ?? 0,
                            'jmlbmenudibeli' => $jmlmenudibeli[0]->total ?? 0
                        ]
                    ); 
    }
